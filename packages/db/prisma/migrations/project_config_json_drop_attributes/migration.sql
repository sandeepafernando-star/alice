-- Project-scoped JSON replaces the standalone attributes table.
ALTER TABLE "projects"
ADD COLUMN "attributes_config" JSONB,
ADD COLUMN "workflow_config" JSONB;

-- Drop reusable attributes table (config now lives on projects).
DROP TABLE IF EXISTS "attributes";

-- Sprint archived status (idempotent on environments that already have it).
ALTER TYPE "SprintStatus" ADD VALUE IF NOT EXISTS 'archived';
