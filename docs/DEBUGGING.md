...
Workspace settings intentionally **do not** include `sonarlint.connectedMode.project` — that key only applies when the [SonarQube for IDE](https://marketplace.visualstudio.com/items?itemName=SonarSource.sonarlint-vscode) extension is installed; otherwise VS Code/Cursor shows **Unknown Configuration Setting**.

Instead:

- **Project binding:** `.sonarcloud.properties` at the repo root (`sonar.organization`, `sonar.projectKey`).
- **Connection token:** configure in **user** settings (not committed), e.g. SonarLint: Connect to SonarCloud.

Install the recommended extension when prompted, or run **SonarLint: Update all bindings to SonarQube/SonarCloud** after connecting.
